 ...
 * Drives Admissions Team's voice via OpenAI or Claude.
...
function buildSystem(lead, campaign, campaignVars = {}) {
  ...
  if (campaignType === 'demo-test-followup') {
    ...
    const base = `You are Annie, an AI Admissions Assistant from Test Prep Pundits. You are on a live phone call with ${lead.fullName} (first name: ${studentFirst}).
...
- "AGENT" in the transcript is the AI admissions counselor (Annie, from Test Prep Pundits) — NOT the student.
...
function buildFollowUpSystem(lead) {
  ...
  return `You are Annie, an AI Admissions Assistant from Test Prep Pundits. You are on a live follow-up phone call.
...
async function detectMeetingFromTranscript(transcript, lead, campaign = null) {
  ...
  const agentRole = isBusiness ? 'HGI business advisor (Ravi)' : 'AI admissions counselor (Annie, from Test Prep Pundits)';

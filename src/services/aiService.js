 ...
 * Drives Antra Kumar's voice via OpenAI or Claude.
...
• DO NOT re-introduce yourself. The call already opened with "Hello, this is Antra from Aiprep365." Never say that again.
...
[END_CALL]      → append ONLY when meeting is booked, caller explicitly declines, or callback is requested`;
  ...
  if (PARENT_TYPES.includes(campaign.type)) {
    return `You are Antra, a warm and professional staff member at Test Prep Pundits. You are on a live phone call with a PARENT.
...
• Use ${studentFirst}'s name naturally in conversation.
• Never use bullet points, numbers, or markdown — spoken words only.
• DO NOT re-introduce yourself. The call already opened with the campaign introduction. Never say "Hello, this is Antra from Aiprep365" again.
...
- "AGENT" in the transcript is the AI admissions counselor (Antra from Aiprep365) — NOT the student.
...
function buildFollowUpSystem(lead) {
  ...
  return `You are Antra, a warm and professional admissions counselor at Aiprep365. You are on a live follow-up phone call.
...
[STEP 1 — GREETING]
Say: "Hello, this is Antra from Aiprep365. May I please speak with ${studentFirst} or their parent?"
...
async function detectMeetingFromTranscript(transcript, lead, campaign = null) {
  ...
  const agentRole = isBusiness ? 'HGI business advisor (Ravi)' : 'AI admissions counselor (Antra from Aiprep365)';

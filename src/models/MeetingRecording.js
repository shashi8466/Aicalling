...
const TABLE = 'meeting_recordings';
...
const MeetingRecording = {
  ...
  async findOne(filter = {}) {
    const rows = await MeetingRecording.find(filter, { limit: 1 });
    return rows[0] || null;
  },
  ...
  async create(data) {
    ...
    const row = { status: 'recording', room_name: '', egress_id: '', file_url: '', storage_path: '', duration_seconds: 0, participant_tracks: [], created_at: now, updated_at: now };
